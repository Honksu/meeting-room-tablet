...
public class DayCalendarVisualizer extends HorizontalScrollView implements ReservatorVisualizer,
    ...
    private PersonalReservation[] personalReservations;
    ...
    public DayCalendarVisualizer(Context context, int dayStartTime, int dayEndTime) {
        super(context, null);
        ...
        this.reservationShader = new LinearGradient(0, 0, 1, 1, getResources().getColor(R.color.CalendarMarkerReservedColor), getResources().getColor(R.color.CalendarMarkerReservedColor), Shader.TileMode.REPEAT);
        markerPaint.setShader(reservationShader);

        this.fadingEdgePaint = new Paint();
        this.leftEdgeShader = new LinearGradient(0, 0, 16, 0, Color.argb(128, 128, 128, 128), Color.argb(0, 0, 0, 0), Shader.TileMode.CLAMP);
        this.rightEdgeShader = new LinearGradient(0, 0, 16, 0, Color.argb(0, 0, 0, 0), Color.argb(128, 128, 128, 128), Shader.TileMode.CLAMP);
    ...
    public synchronized void setPersonalReservations(List<PersonalReservation> reservationList) {
        long start = System.currentTimeMillis();
        this.personalReservations = new PersonalReservation[reservationList.size()];
        reservationList.toArray(this.personalReservations);
        Arrays.sort(this.personalReservations);
        generateDayHeaderLabels();
        contentFrame.setPadding(Math.max(getWidth(), daysToShow * dayWidth + timeLabelWidth), 0, 0, 0);
        Log.d("Performance", "Set reservations done in " + (System.currentTimeMillis() - start) + "ms");
    }
    ...
    private void generateDayHeaderLabels() {
        if (personalReservations.length > 0) {
            dayLabels = new String[Math.max(
                    getDaysFromStart(personalReservations[personalReservations.length - 1]
    ...
    private void drawCalendarReservations(Canvas c, RectF area) {
        ...
        if (personalReservations.length > 0) {
            float[] points = new float[personalReservations.length * 8];
            short[] indices = new short[personalReservations.length * 6];
            for (int i = 0; i < personalReservations.length; i++) {
                int j = 8 * i;
                //order of points is top-left, top-right, bottom-left, bottom-right
                points[j] = getXForTime(personalReservations[i].getStartTime());
                points[j + 1] = getProportionalY(personalReservations[i].getStartTime()) * height;
                points[j + 2] = getXForTime(personalReservations[i].getStartTime()) + dayWidth;
                points[j + 3] = points[j + 1];
                points[j + 4] = points[j];
                points[j + 5] = getProportionalEndY(personalReservations[i].getEndTime()) * height;
            ...
            c.drawVertices(Canvas.VertexMode.TRIANGLE_STRIP, points.length, points, 0,
            ...
            for (int i = 0; i < personalReservations.length; i++) {
                if ((i + 1) < personalReservations.length &&
                        personalReservations[i].getEndTime().getTimeInMillis() == personalReservations[i + 1].getStartTime().getTimeInMillis()) {
                    c.drawLine(getXForTime(personalReservations[i].getStartTime()),
                            getProportionalEndY(personalReservations[i].getEndTime()) * height,
                            getXForTime(personalReservations[i].getStartTime()) + dayWidth,
                            getProportionalEndY(personalReservations[i].getEndTime()) * height,
                            linePaint);
                }
            }
    ...
    private void drawReservationSubjects(Canvas c, RectF area) {
        ...
        for (PersonalReservation r : personalReservations) {
    ...
    private PersonalReservation getReservationForTime(DateTime time) {
        for (int i = 0; i < personalReservations.length; i++) {
            if (personalReservations[i].getStartTime().before(time)) {
                if (personalReservations[i].getEndTime().after(time)) {
                    return personalReservations[i];
                }
            } else {
                return null;
            }
    ...
    private Reservation findReservationBefore(DateTime time) {
        ...
        for (int i = 0; i < personalReservations.length; i++) {
            if (personalReservations[i].getEndTime().before(time)) {
                latest = personalReservations[i];
            } else {
                return latest;
            }
        }
    ...
    private PersonalReservation findReservationAfter(DateTime time) {
        for (int i = 0; i < personalReservations.length; i++) {
            if (personalReservations[i].getStartTime().after(time)) {
                return personalReservations[i];
            }
        }
        return null;
    ...
    private DateTime getFirstDayToShow() {
        if (personalReservations.length > 0
                && personalReservations[0].getStartTime().before(firstDayToShow)) {
            return personalReservations[0].getStartTime();
        } else {
    ...
    public void setOnClickListener(final OnClickListener l) {
        ...
        contentFrame.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                l.onClick(DayCalendarVisualizer.this);
            }
        });
    }
